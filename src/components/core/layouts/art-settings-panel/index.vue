      ...
      <div class="drawer-con">
        ...
        <div class="theme-styles">
          ...
          >
            ...
            <p class="name">{{ themeList[index] }}</p>
          ...
          <div class="menu-layouts">
            <div class="menu-layouts-wrap">
              ...
              >
                ...
                <p class="name">{{ menuTypeList[index] }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="box-style">
          ...
          <div class="button" :class="{ 'is-active': boxBorderMode }" @click="switchBoxMode(false, 'border-mode')">
            边框
          </div>
          <div class="button" :class="{ 'is-active': !boxBorderMode }" @click="switchBoxMode(false, 'shadow-mode')">
            阴影
          </div>
        </div>
        ...
        <div class="basic-box">
          ...
          <div class="item" style="display: flex">
            ...
            <el-select
              ...
              @change="setTabStyle"
            >
              <el-option v-for="item in tabStyleOps" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </div>
          <div class="item" style="display: flex">
            <span>页面切换动画</span>
            <el-select v-model="pageTransition" size="default" style="width: 120px" @change="setPageTransition">
          ...
          <div class="item" style="display: flex">
            ...
            >
              <el-option v-for="item in customRadiusOps" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </div>
        </div>
      </div>
...
const menuTypeList = ['垂直', '水平', '混合', '双列']
const themeList = ['浅色', '深色', '系统']
...
const autoCloseHandler = (storeMethod: (...args: any[]) => void, needReload: boolean = false, ...args: any[]) => {
  storeMethod(...args)
  if (needReload) {
    settingStore.reload()
  }
...
const showWorkTabFunc = () => autoCloseHandler(settingStore.setWorkTab, false, !settingStore.showWorkTab)

const setPageTransition = (transition: string) => autoCloseHandler(settingStore.setPageTransition, false, transition)

const setTabStyle = (style: string) => autoCloseHandler(settingStore.setTabStyle, false, style)

const setContainerWidth = (type: ContainerWidthEnum) => autoCloseHandler(settingStore.setContainerWidth, true, type)

const setElementTheme = (theme: string) => autoCloseHandler(settingStore.setElementTheme, true, theme)

const setCustomRadius = (radius: string) => autoCloseHandler(settingStore.setCustomRadius, false, radius)
