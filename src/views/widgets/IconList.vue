    ...
    <div class="list">
      <ul class="icon-list">
        <li v-for="icon in systemIconClasses" :key="icon.className" @click="copyIcon(icon)">
          <i class="iconfont-sys" v-if="iconType === 'unicode'" v-html="icon.unicode" :style="getIconStyle()"></i>
...
const copyIcon = (text: IconfontType) => {
  ...
  copyipt.setAttribute('value', (iconType.value === 'unicode' ? text.unicode : text.className) || '')
